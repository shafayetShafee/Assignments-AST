...
classoption: oneside
geometry: [top=1.1in, bottom=1in, right=1.1in, left=1.1in]
linestretch: 1.25
fontsize: 12pt
---
...
mod_params <- function(mod, param, dec = 2) {
...
reg_eq(m1)
```
Then, the cholesterol level is expected to increase by `r mod_params(m1, 2)` units
...
reg_eq(m2)
```
So, we can say that, Mean cholesterol level of `Type A dibpat` subjects is `r mod_params(m2, 1)`. Also, mean cholesterol level of `Type A dibpat` subjects is `r mod_params(m2, 2)` unit higher than `Type B dibpat` subjects.
